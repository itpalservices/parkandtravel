...
export class BookingFormComponent implements OnInit, OnDestroy {
  ...
  private async showParkPlaceAndImagesModal(newStatusId: string, newStatusLabel: string): Promise<void> {
    ...
    if (finalPrice === null) {
      ...
      const customerEmail = this.existingBooking?.email || '';
    ...
    Swal.fire({
      ...
      preConfirm: async () => {
        ...
        if (!/^[A-Z]-\d{3}$/.test(parkPlace)) {
          Swal.showValidationMessage('Parking place must be in format A-000 (e.g., A-001)'); return false;
        }
        try {
              ...
              next: (res: any) => {
                ...
                if (receiptId) {
                  Swal.fire({
                    ...
                    if (r.isConfirmed) {
                      await this.openThermalReceipt(receiptId, this.bookingId!);
                    }
  ...
  private async openThermalReceipt(receiptId: string, bookingId: string): Promise<void> {
  ...
  private handleImageFiles(
    ...
    newFiles.forEach((file) => {
      ...
      if (file.size > BookingFormComponent.MAX_RAW_IMAGE_SIZE) {
  ...
  private async handleCompletedStatus(_newStatusId: string, _newStatusLabel: string): Promise<void> {
    ...
    const isTbc = !this.existingBooking.dateTo;
    ...
    const finalPrice = isTbc ? tbcFinalPrice! : (this.existingBooking.finalPrice ?? 0);
    const totalAmount = finalPrice - (this.existingBooking.waivedAmount ?? 0) + (applyExtraFee ? extraFee : 0);
    ...
    if (remainingBalance < -0.001) {
      ...
      const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
          ...
          return { amount: enteredAmount, paymentMethod: selectedMethod, sendEmail, isDiscount: discountConfirmed };
        },
      });
      ...
      next: (res: any) => {
        ...
        if (receiptId) {
          Swal.fire({
            ...
            if (r.isConfirmed) {
              this.openThermalReceipt(receiptId, this.bookingId!);
            }
            this.router.navigate(['/admin/bookings']);
          });
        } else {
